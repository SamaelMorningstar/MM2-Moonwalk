Loadscript:

loadstring(game:HttpGet('https://raw.githubusercontent.com/SamaelMorningstar/MM2-Moonwalk/refs/heads/main/MM2-Moonwalk.lua'))()

# > MAIN Tab
- Aimbot Target Role Selection (Sheriff/Murderer)
- Spectate Mode
- Lock Camera
- Autofarm
- Teleport to Lobby
- Fling All Players
- Click Teleport
- Building Tools

# > ESP Tab
- X-Ray Vision
- Murderer ESP
- Innocent ESP
- Sheriff ESP
- GunDrop ESP

# > PLAYER Tab
- Walkspeed Adjustment
- JumpPower Adjustment
- Gravity Adjustment
- FOV (Field of View) Adjustment
- Infinite Jump
- Noclip
- Fly
- Fly Speed Adjustment
- Restore Default Settings

# > INNOCENT Tab
- Get Dropped Gun
- Auto-Get Dropped Gun
- Get Gun and Shoot Murderer
- Insta Win (Fling Murderer)

# > MURDERER Tab
- Aimbot (Lock Sheriff)
- Player Selection for Kill
- Refresh Player List
- Kill Selected Player
- Kill All Players Toggle

# > SHERIFF Tab
- Shot Type Selection (Ping/Teleport)
- Shoot Murderer
- Aimbot (Lock Murderer)
- Insta Win

# > TELEPORT Tab
- Player Selection for Teleport
- Teleport to Selected Player
- Update Players List
- Teleport to Lobby
- Teleport to Sheriff
- Teleport to Murderer
- Teleport to Random Player

# > SETTINGS Tab
- Boost FPS
- Full Brightness
- Anti AFK
- Rejoin Server
- Server Hop
- Join Lower Server (Less Players)
